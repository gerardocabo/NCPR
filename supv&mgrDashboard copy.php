...
<body class="bg-white">
    ...
    <div class="modal fade" id="dispoModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                ...
                <div class="dispo modal-body">
                    <!-- Content will be loaded here include "disposition.php"; -->
                    <?php  ?>
                </div>
    ...
    <script>
        ...
        function fetchDispoDetails(ncprNum, viewOnly) {
            $.ajax({
                url: 'fetch_dispo_detailsUpper.php',
                method: 'POST', // Use GET to match PHP script
                data: {
                    ncpr_num: ncprNum,
                    viewOnly: viewOnly,
                },
                success: function(response) {
                    $('#dispoModal .modal-body').html(response); // Insert HTML response into modal
                    $('#dispoModal').modal('show'); // Show modal
                },
                error: function() {
                    alert('Error fetching data.');
                }
        ...
        // Attach event listener to button
        $(document).on('click', '.dispo-btn', function() {
            var ncprNum = $(this).data('id');
            fetchDispoDetails(ncprNum, true);
        });
    </script>
